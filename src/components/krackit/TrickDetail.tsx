...
import {
  ...
  ChevronLeft, ChevronRight, Copy, Share2, Sparkles,
} from "lucide-react";
    ...
    >
      <header className="flex items-center justify-between px-5 pb-3 pt-5">
        ...
        {trickList.length > 1 && (
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            <button
              onClick={() => hasPrev && onNavigate(trickList[currentIndex - 1])}
              disabled={!hasPrev}
            ...
            <button
              onClick={() => hasNext && onNavigate(trickList[currentIndex + 1])}
              disabled={!hasNext}
              className="flex h-8 w-8 items-center justify-center rounded-full bg-surface disabled:opacity-30"
            >
        ...
        <div className="flex gap-2">
          ...
          >
            {shareLabel ? (
              <><Copy className="h-3.5 w-3.5" />{shareLabel}</>
            ) : (
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-8">
        ...
        <div className="mt-3 rounded-2xl border border-border bg-surface p-4">
          {rating === 0 ? (
            <>
              <p className="mb-3 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground text-center">Rate this trick</p>
            ...
            <>
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground text-center">Your rating</p>
              ...
              {avgRating && (
                <div className="mt-3 flex items-center justify-center gap-2 rounded-xl bg-gold/10 px-4 py-2">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="#D4A24C" stroke="#D4A24C" strokeWidth="1.5">
                    <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" />
                  </svg>
