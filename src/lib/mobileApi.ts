...
export type Exam       = { id: string; name: string; short: string; description: string | null; subjects_count: number; tricks_count: number; accent: string; is_active: boolean; sort_order: number };
...
export type ExamPricing= { exam_id: string; monthly: number; sixmonths: number; yearly: number };
...
export type Subscription={ id: string; user_id: string; exam_id: string | null; plan_type: string; billing_cycle: BillCycle; amount_paise: number; status: string; expires_at: string };
export type AppSettings = { app_name: string; tagline: string; version: string; description: string | null; support_email: string | null; website_url: string | null; team_members: {name:string;role:string}[]; social_links: {platform:string;url:string}[] };
...
export const contentApi = {
  async loadAll() {
    const [exams, subjects, chapters, topics, tricks, notes, news, pricing] = await Promise.all([
      ...
      supabase.from('tricks').select('*').order('sort_order').then(r => ok(r.data, r.error)),
  ...
  async getTricksOfDay(): Promise<{ trick: Trick; note: string | null }[]> {
    const today = new Date().toISOString().slice(0, 10);
    const { data } = await supabase
      .from('trick_of_day')
      .select('note, trick:tricks(*)')
      .eq('date', today)
      .order('id')
      .limit(3);
    ...
    return (data as any[])
      ...
      .map((row) => ({ trick: row.trick as Trick, note: row.note as string | null }));
  },
