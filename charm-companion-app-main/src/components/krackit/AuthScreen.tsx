...
export function AuthScreen({ onAuth }: { onAuth: (name: string, email: string) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        // onAuthStateChange in index.tsx will detect the new session automatically
        onAuth(sanitize(name), sanitize(email).toLowerCase());
      } else {
        await mobileAuth.signIn(sanitize(email).toLowerCase(), password);
        onAuth(sanitize(email).split("@")[0], sanitize(email).toLowerCase());
      }
    } catch (err: any) {
    ...
    <div className="flex flex-1 flex-col overflow-y-auto bg-background px-6 pb-10 pt-10">
      ...
      <div className="mb-8 flex flex-col items-center gap-4">
        <div className="relative">
          ...
          <div className="relative flex h-20 w-20 items-center justify-center rounded-3xl border border-gold/30 bg-gradient-to-br from-gold/20 to-transparent">
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className="flex items-center justify-center gap-2 rounded-xl border border-border bg-surface py-3 text-sm font-medium text-foreground transition-colors hover:border-gold/30 active:scale-[0.98]"
        >
        ...
        <button
          onClick={() => mobileAuth.signInWithApple().catch((e) => setGlobalError(e.message))}
          className="flex items-center justify-center gap-2 rounded-xl border border-border bg-surface py-3 text-sm font-medium text-foreground transition-colors hover:border-gold/30 active:scale-[0.98]"
        >
